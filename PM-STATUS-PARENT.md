...
| T20 | Socket emitters (`ticket:*` + `verification:*` + `notification:new`)             | B    | Nathan  | wip      | —           | PLAN ACK'd 2026-07-02. Q-B-14 → `shared/socket/` (Slot-B-ownable, no core/ gate → no escalation). DEP-7 (live io server = foundation) flagged §10. Wire 4/8 events (4 have Slot-B producers) + builders for rest. No dep added (structural type). Coding `feat/socket-emitters`. |
