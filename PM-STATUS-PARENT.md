...
| T09 | CSV import utility (used by menu + knowledge)                                    | A    | Nanak   | assigned | —           | Issued 2026-07-02 H0 post-T08 merge. Main-queue continuation per PO direct straight-line direction (no triage cycle). Ships pure-fn `parseCsvWithSchema<T>` at `src/shared/utils/csv-parser.ts` (mirrors T06 pattern). Hand-rolled parser handles practical CSV subset (quoted fields, escaped quotes, LF/CRLF/CR, UTF-8 BOM); zod schema-driven per-row validation; batched error surface (no throw-on-first per PO input). No new deps. Streaming deferred to slice-2. |
