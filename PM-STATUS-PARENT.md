...
| T-INFRA-02 | Foundation: DEP-5 fix — add `userId: string` to `TenantContext` + `deriveTenantContext` | A    | Nanak   | assigned | —           | Issued 2026-07-02 H0 post-T06 merge. Addresses PARENT §10 DEP-5 escalated by PM B. **Nathan claimed "observed SHIPPED" in H13 roll-up but PM A independent read of `tenant-guard.ts` shows userId is still MISSING** — Nathan's observation was inaccurate. Small: 2-3 line change in `tenant-guard.ts` + test fixture updates. Unblocks Nathan's T19 (notifications = per-user scope). |
