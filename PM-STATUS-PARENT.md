...
| T-INFRA-03 | Foundation: GAP-T11-3 fix — split `test:unit` from integration tests so `make check` stays Docker-free | A    | Nanak   | assigned | —           | Issued 2026-07-02 H0 post-T-INFRA-02 merge. Addresses PARENT §3b GAP-T11-3 escalated by PM B. Current `package.json:25` `test:unit` regex `__tests__/.*\.test\.ts` matches both regular + integration tests → `make check` currently spins Docker. Fix: add `--testPathIgnorePatterns='\.integration\.test\.ts$'` to `test:unit`. ~1 LOC. Non-blocking but improves everyone's `make check` speed. |
