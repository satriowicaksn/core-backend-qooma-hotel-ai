...
| T20 | Socket emitters (`ticket:*` + `verification:*` + `notification:new`)             | B    | Nathan  | assigned | —           | Issued 2026-07-02 (LAST Slot-B task). `SocketEmitterPort`+adapter + wire seams. **Q-B-14** (port location, may touch `core/`) + **DEP-7** (io server = foundation/DEP-4). Awaiting PLAN. |
