...
| T-INFRA-02 | Foundation: DEP-5 fix — add `userId: string` to `TenantContext` + `deriveTenantContext` | A    | Nanak   | approved | PM A (Nanak) | ✅ APPROVED attempt 1 (2026-07-02 H0). `feat/foundation-userid-tenant-context` @ `d0a0bcc` — **awaiting PO merge**. 10 files (3 Slot A source/test + 7 Slot B test fixtures via PM A GAP-1 Path B scope override — type-breaking cross-slot required single-PR execution). 191 tests pass on branch (+1 new). 100% coverage on `tenant-guard.ts`. All 4 Path B guardrails independently verified. Nathan self-corrected his DEP-5 misclaim in `438664d` (line 118 + T19 row). **T19 + T12 actor_user_id (Q-B-11) unblocked** post-merge. |
...
| T12 | Ticket status transition + reroute (state-machine-validated)                     | B    | Nathan  | assigned | —           | **UNBLOCKED 2026-07-02** (T06 ✅ + DEP-6 ✅ merged). Issued by PM B. Consumes `ticket-state-machine.ts` + `BusinessRuleError`. Soft-dep DEP-5 for `actor_user_id` (Q-B-11). Awaiting PLAN. |
...
| T19 | Notifications CRUD + optimistic ops                                              | B    | Nathan  | assigned ⛔ | —          | ⚠ CORRECTION 2026-07-02: **STILL BLOCKED on DEP-5** — `TenantContext` lacks `userId` (verified; T-INFRA-02 assigned, NOT merged). My earlier "unblocked" roll-up was a mis-read. |
