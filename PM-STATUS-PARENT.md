...
| T05 | Seed scripts (1 demo hotel via Auth API + 5 depts + sample menu + KB)            | A    | Nanak   | assigned | —           | Issued 2026-07-02 H0 post-T-INFRA-03 merge (PO strict main-queue directive). **Opsi C twist** — S2 (runtime Prisma upsert, env-configurable `SEED_HOTEL_ID`) default per silent-ratification of PARENT §10 T05 coord Q (PM B silent, ~1 session cycle window elapsed). PM B welcome to review/override at SUBMIT stage. Modifies existing `prisma/seeds/index.ts` stub. |
...
| T12 | Ticket status transition + reroute (state-machine-validated)                     | B    | Nathan  | approved | PM B (Nathan) | ✅ APPROVED attempt 1 (2026-07-02). PM rerun: make check 173 (no-Docker) + coverage 96.68% + drift clean + merge dry-run CLEAN + 422/403 negatives + optimistic-concurrency race-check. `feat/tickets-transition` @ `625a8b8` → **awaiting PO merge**. |
