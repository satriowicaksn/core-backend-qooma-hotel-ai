...
| T10 | Workers harness (cron + queue) — actual workers wired per B/C tasks              | A    | Nanak   | assigned | —           | Issued 2026-07-02 H0 post-T09 merge. **Last main-queue T01-T10 task**. Implements existing `src/core/queue/bull-factory.ts` stub (boilerplate placeholder → real impl, matches T-INFRA-01 pattern). Slice-1: `createQueue<TData>` + `registerWorker` + `shutdownAllQueues` + SIGTERM/SIGINT handler (T-INFRA-01 `void`+`NODE_ENV !== 'test'` pattern). No new deps (bull 4.16 + ioredis 5.3 already present). PO ratified naming `<domain>.<action>` (updates boilerplate JSDoc). `redis-client.ts` stub untouched (separate concern for future cache/rate-limit task). Port+adapter and real Redis integration test deferred to slice-2. |
