...
| T07-slice-1 | Foundation: `BusinessRuleError` (422) — first slice of T07 error-handler build-out (DEP-6 fix) | A    | Nanak   | assigned | —           | Issued 2026-07-02 H0. Addresses PARENT §10 DEP-6 escalated by PM B — envelope shape ratified by PM B (`code = 'BUSINESS_RULE'` + `details.rule`). **Unblocks T12 (ticket transition) + T16-V2..5 (visit verify-manual)** — 5 Slot B tasks. Rest of T07 (7 other spec §7 codes) deferred to future slices on consumer demand. |
