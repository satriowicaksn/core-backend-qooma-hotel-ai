...
| T06 | Ticket state-machine helper + unit-test the transition table                     | A    | Nanak   | assigned | —           | Issued 2026-07-02 H0. Direct dependency of Nathan's T12 (ticket status transition). Codifies spec §5 state machine (8 states, ~15 transitions, 2 terminal). Uses T07-slice-1's `BusinessRuleError` with `rule: 'INVALID_TICKET_TRANSITION'` per spec §5 line 74. Home: `src/shared/utils/ticket-state-machine.ts` (pure fn per CLAUDE.md §3). |
...
| T07-slice-1 | Foundation: `BusinessRuleError` (422) — first slice of T07 error-handler build-out (DEP-6 fix) | A    | Nanak   | approved | PM A (Nanak) | ✅ APPROVED attempt 1 (2026-07-02 H0). `feat/foundation-business-rule-error` @ `b214743` — **awaiting PO merge**. 2 files (app-errors.ts pure append + fresh test file, 6 new tests: 4 BusinessRuleError + 2 bonus sanity). 150 tests pass on branch (+6 vs 144 baseline). Envelope shape verified via test — matches PM B ratified contract (`code = 'BUSINESS_RULE'` + `details.rule`). 9 existing classes untouched (`git diff` pure `+` verified). Cherry-pick transparency clean. §10 DEP-6 → resolved. |
...
| 2026-07-02 | **DEP-6 — no `422 BusinessRuleError` in `core/errors`**: blocks T16-V2..5 (visit verify-manual) **and T12** (ticket transition) — both need `422 BUSINESS_RULE` for invalid transitions. | B (T16, T12 blocked), A (owns `core/errors`, T07) | **RESOLVED 2026-07-02 H0 via T07-slice-1** (`feat/foundation-business-rule-error` @ `b214743`, PM A APPROVED — awaiting PO merge). Class shipped per PM B's ratified envelope. Post-merge: T12 + T16-V2..V5 unblocked. Rest of T07 (7 other spec §7 codes) deferred to future slices on consumer demand — envelope-generic + `details.rule` approach means no additional subclasses needed until a wire-level discriminator becomes necessary. |
